...
export const FormTodo = () => {
    ...
    const HandlingSubmitting =(e)=>{
        e.preventDefault ()
        console.log(new Date(pred).getDate() <= new Date().getDate() && new Date(pred).getMonth() <= new Date().getMonth() && new Date(pred).getFullYear() <= new Date().getFullYear() )
        //for checken day and mounth and year wbefore adding todo so most be grathen that today 
        if(new Date(pred).getDate() >= new Date().getDate() && new Date(pred).getMonth() >= new Date().getMonth() && new Date(pred).getFullYear() >= new Date().getFullYear() ){
